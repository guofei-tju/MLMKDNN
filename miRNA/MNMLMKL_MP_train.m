function [Y,err_list,betas1,betas1f,betas2,betas2f] = MNMLMKL_MP_train(K1_list,K2_list,...
          y_train,nLayers,lambda,maxI,LR1,LR2,sig1,sig2,p2,p3,N_features1,N_features2,num_nodes)
%MNMLMKL_MP MultiNetwork-multiLayer-Multikernel-MultiProjection
%%%%%%%%%%%%%%%%%%%%%%%%%
%   intruduction:
%   each target and drug generate four network using four features respectively，
...
%miRNA side
for i = 1:N_features1   
    [KNet1(:,:,:,i),KCOMList1(:,:,i)] = MN_computeKernels(K1_list(:,:,i),betas1(:,:,i),num_nodes,nLayers,sig1,p2,p3);
...
% disease side 
for i = 1:N_features2
    [KNet2(:,:,:,i),KCOMList2(:,:,i)] = MN_computeKernels(K2_list(:,:,i),betas2(:,:,i),num_nodes,nLayers,sig2,p2,p3);
...
for i = 1:maxI
    ...
    if nLayers==3
         [betas1,betas1f] = grad_3_Layer(betas1,betas1f,KCOMList1,KNet1,...
                                          KCOMList1_all,KCOMList1f,K2_final,...
                                          Theta,y_train,LR1,sig1,p2,p3,N_features1,0);
                           
         [betas2,betas2f] = grad_3_Layer(betas2,betas2f,KCOMList2,KNet2,...
                                          KCOMList2_all,KCOMList2f,K1_final,...
                                          Theta,y_train,LR2,sig2,p2,p3,N_features2,1);                                             
    end
%% forward calculation
    %miRNA side
    for i = 1:N_features1   
